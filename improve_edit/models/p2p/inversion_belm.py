...
class NullInversion:
    ...
    @torch.no_grad()
    def belm_loop(self, latent):
        uncond_embeddings, cond_embeddings = self.context.chunk(2)
        all_latent = [latent]
        latent = latent.clone().detach()
        for i in range(self.num_ddim_steps):
            index = self.num_ddim_steps - i - 1
            time = self.model.scheduler.timesteps[index + 1] if index < self.num_ddim_steps - 1 else 1
            noise_pred = self.get_noise_pred_single(latent, time, cond_embeddings)
            if index < self.num_ddim_steps - 1:
                alpha_i = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index]].to(torch.float32)
                alpha_i_minus_1 = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index + 1]].to(torch.float32)
            else:
                alpha_i = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index]].to(torch.float32)
                alpha_i_minus_1 = 1
            ...
            if i == 0:
                latent = (alpha_i / alpha_i_minus_1) * latent + (
                            sigma_i - (alpha_i / alpha_i_minus_1) * sigma_i_minus_1) * noise_pred
            else:
                alpha_i_minus_2 = 1 if i == 1 else self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index + 2]].to(
                    torch.float32)
                ...
                latent = coef_x_i_minus_2 * all_latent[-2] + coef_x_i_minus_1 * all_latent[-1] + coef_eps * noise_pred
            all_latent.append(latent)
        return all_latent
    ...
    @torch.no_grad()
    def belm_inversion(self, image):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents = self.belm_loop(latent)
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5):
        ...
        image_rec, ddim_latents = self.belm_inversion(image_gt)
...
class DirectInversion:
    ...
    def belm_loop(self, latent):
        ...
        for i in range(self.num_ddim_steps):
            ...
            if index < self.num_ddim_steps - 1:
                alpha_i = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index]].to(torch.float32)
                alpha_i_minus_1 = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index + 1]].to(
                    torch.float32)
            else:
                alpha_i = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index]].to(torch.float32)
                alpha_i_minus_1 = 1
            ...
            if i == 0:
                ...
                alpha_i_minus_2 = 1 if i == 1 else self.model.scheduler.alphas_cumprod[
                    self.model.scheduler.timesteps[index + 2]].to(
                    torch.float32)
                sigma_i_minus_2 = (1 - alpha_i_minus_2) ** 0.5
                alpha_i_minus_2 = alpha_i_minus_2 ** 0.5
    ...
    @torch.no_grad()
    def belm_null_loop(self, latent):
        uncond_embeddings, cond_embeddings = self.context.chunk(2)
        ...
        for i in range(self.num_ddim_steps):
            index = self.num_ddim_steps - i - 1
            time = self.model.scheduler.timesteps[index + 1] if index < self.num_ddim_steps - 1 else 1
            noise_pred = self.get_noise_pred_single(latent, time, uncond_embeddings)
            if index < self.num_ddim_steps - 1:
                alpha_i = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index]].to(torch.float32)
                alpha_i_minus_1 = self.model.scheduler.alphas_cumprod[self.model.scheduler.timesteps[index + 1]].to(
                    torch.float32)
    ...
    @torch.no_grad()
    def belm_with_guidance_scale_loop(self, latent,guidance_scale):
        uncond_embeddings, cond_embeddings = self.context.chunk(2)
        uncond_embeddings=uncond_embeddings[[0]]
        cond_embeddings=cond_embeddings[[0]]
        all_latent = [latent]
        ...
        for i in range(self.num_ddim_steps):
            index = self.num_ddim_steps - i - 1
            time = self.model.scheduler.timesteps[index + 1] if index < self.num_ddim_steps - 1 else 1
            uncond_noise_pred = self.get_noise_pred_single(latent, time, uncond_embeddings)
            cond_noise_pred = self.get_noise_pred_single(latent, time, cond_embeddings)
    ...
    @torch.no_grad()
    def belm_null_inversion(self, image):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents = self.belm_null_loop(latent)
        return image_rec, ddim_latents
    
    @torch.no_grad()
    def belm_with_guidance_scale_inversion(self, image,guidance_scale):
        latent = image2latent(self.model.vae, image)
        image_rec = latent2image(self.model.vae, latent)[0]
        ddim_latents = self.belm_with_guidance_scale_loop(latent,guidance_scale)
        return image_rec, ddim_latents
    ...
    def invert(self, image_gt, prompt, guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5):
        ...
        image_rec, belm_latents = self.belm_inversion(image_gt)
        
        noise_loss_list = self.offset_calculate(belm_latents, num_inner_steps, early_stop_epsilon,guidance_scale)
        return image_gt, image_rec, belm_latents, noise_loss_list
    ...
    def invert_with_guidance_scale_vary_guidance(self, image_gt, prompt, inverse_guidance_scale, forward_guidance_scale, num_inner_steps=10, early_stop_epsilon=1e-5):
        ...
        image_rec, belm_latents = self.belm_with_guidance_scale_inversion(image_gt,inverse_guidance_scale)
        
        noise_loss_list = self.offset_calculate(belm_latents, num_inner_steps, early_stop_epsilon,forward_guidance_scale)
        return image_gt, image_rec, belm_latents, noise_loss_list
